...
const CARD_GRADIENTS = [
  "from-indigo-500/20 via-zinc-900 to-zinc-900",
  "from-cyan-500/20 via-zinc-900 to-zinc-900",
  "from-indigo-500/20 via-zinc-900 to-zinc-900",
  "from-indigo-600/20 via-zinc-900 to-zinc-900",
  "from-cyan-400/18 via-zinc-900 to-zinc-900",
  "from-indigo-400/18 via-zinc-900 to-zinc-900",
];
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${
                    isActive
                      ? "border-indigo-400 bg-indigo-400 text-zinc-950 shadow-lg shadow-indigo-500/20"
                      : "border-zinc-700 bg-zinc-950/60 text-zinc-300 hover:border-indigo-400/60 hover:text-indigo-300"
                  }
                `}
              >
                {tab.label}
  ...
  return (
    <article
      className="
        ...
        hover:-translate-y-2 hover:border-indigo-400 hover:shadow-2xl hover:shadow-indigo-500/10
      "
    >
      ...
      <div className={`relative h-44 w-full overflow-hidden bg-gradient-to-br ${gradient}`}>
        ...
        <div className="pointer-events-none absolute -right-10 -top-10 h-40 w-40 rounded-full bg-indigo-400/10 blur-3xl transition-opacity duration-500 group-hover:opacity-80" />
        ...
        <span className="absolute bottom-4 left-4 rounded-md bg-zinc-950/70 px-2.5 py-1 font-mono text-[11px] uppercase tracking-widest text-indigo-300 backdrop-blur-sm">
          {project.tag}
        </span>

        {/* Status badge — top-right */}
        <span className="absolute right-4 top-4 rounded-full border border-indigo-400/30 bg-zinc-950/70 px-3 py-1 text-xs font-medium text-indigo-300 backdrop-blur-sm">
          {project.status}
        </span>
      </div>
      ...
      <div className="flex flex-1 flex-col p-6">
        <h3 className="text-xl font-bold text-zinc-50 transition-colors duration-300 group-hover:text-indigo-400">
          {project.title}
        </h3>
            ...
            <>
              {project.github ? (
                <a
                  ...
                  className="flex items-center gap-1.5 text-sm text-zinc-400 transition-colors hover:text-indigo-400"
                >
                  <Code2 size={16} />
                  Code
                </a>
              ...
              {project.demo && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group/link flex items-center gap-1.5 text-sm font-semibold text-indigo-400"
                >
                  Live Demo
                  <ArrowUpRight
                    size={16}
